...
{
    element_width* currentElement, i;
    /*Check if the FIFO buffer is valid*/
    if (!fifo_buf || !fifo_buf->head || !fifo_buf->base || !fifo_buf->tail)
        return FIFO_NULL;
    ...
    currentElement = fifo_buf->tail;

    for (i = 0; i < fifo_buf->size; i++)
    {
        printf("%d\n", *currentElement);

        if (currentElement == (fifo_buf->base + (fifo_buf->capacity - 1)))
            currentElement = fifo_buf->base;
        else
            currentElement++;
    }    
